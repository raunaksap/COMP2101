...
# we pull the desired interface using a command like ip route, and if no interface is found, we default to ens33
interfaceN=($(ip route | grep -w "default" | awk '{print $5}'))
